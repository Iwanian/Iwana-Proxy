...
enum BannerLoader {
    ...
    private static func fetchRemote() async -> [BannerItem]? {
        do {
            ...
            // GitHub's API rejects any request with no User-Agent (403
            // "Request forbidden by administrative rules") — this was the
            // bug stopping the banner from ever loading.
            request.setValue("IwanaProxy-macOS", forHTTPHeaderField: "User-Agent")
